...
public class ChooseAreaActivity extends Activity{
    ...
    private TodayWeatherDB todayWeatherDB;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        todayWeatherDB = TodayWeatherDB.getInstance(this);
    ...
    private void queryProvinces(){
        provinceList = todayWeatherDB.loadProvinces();
    ...
    private void queryCities(){
        cityList=todayWeatherDB.loadCities(selectedProvince.getId());
        if (cityList.size()>0){
            dataList.clear();
    ...
    private void queryCounties(){
        countyList = todayWeatherDB.loadCounties(selectedCity.getId());
        ...
        HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
            ...
            public void onFinish(String response) {
                ...
                if ("province".equals(type)){
                    result = Utility.handleProvinceResponse(todayWeatherDB,response);
                }else if ("city".equals(type)){
                    result = Utility.handleCitiesResponse(todayWeatherDB,response,selectedProvince.getId());
                }else if ("county".equals(type)){
                    result = Utility.handleCountiesResponse(todayWeatherDB,response,selectedCity.getId());
                }
                if (result){
    ...
    /**
     * 捕获back键，根据当前级别来判断，此时应该是返回市列表，省列表，还是直接退出
     */
    @Override
